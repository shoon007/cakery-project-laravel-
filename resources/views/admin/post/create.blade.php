...
@section('content')
    ...
    <div class="main">
        ...
        <div class="edit-container">
            <form action="{{ route('product#create') }}" method="post" enctype="multipart/form-data" class="edit-form">
                @csrf
                <div class="profile-div">
                    <img src="{{ asset('img/img-default.webp') }}" alt="" class="img-thumbnail post-img"
                        id="profile-img">
                    <div class="upload">
                ...
                <div class="d-flex flex-column align-content-center justify-content-center">
                    <div class="inputs-container">
                        <div class="inputs">
                            <label for="">Name<br>
                                <input type="text" placeholder="Enter name" name="postName" value="{{ old('postName') }}"
                                    id="name">
                                @error('postName')
                                ...
                                <select name="category" class="form-select my-custom-select"
                                    ...
                                    @foreach ($categories as $item)
                                        <option value="{{ $item->id }}"
                                            @if (old('category') == $item->id) selected="selected" @endif>{{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('category')
                        ...
                        <div class="inputs">
                            <label for="">Description<br>
                                <textarea id="textarea" cols="30" rows="4" name="description" placeholder="Enter description">
                                    {{ old('description') }}
                                </textarea>
                                @error('description')
                                    <small class="text-danger ms-2 d-block">{{ $message }}</small>
                    ...
                    <button class="button mt-3">Create
                    </button>
                </div>
            </form>
